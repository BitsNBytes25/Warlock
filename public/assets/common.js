...
/**
 * SHA256 encode a string, used as a fallback if crypto is unavailable due to the site not running in HTTPS mode.
 * @param ascii
 * @returns {string}
 */
function legacy_sha256(ascii) {
	...
	//* caching results is optional since local variables are fast enough
	...
	ascii += '\x80' // Append Ɨ to the end of the message
...
function sha256(str) {
	if (crypto.subtle === undefined) {
		// Fallback required if the site is not running in HTTPS.
		// This is allowed because Warlock may be installed on a local network for local access.
		return new Promise((resolve, reject) => {
			resolve(legacy_sha256(str));
		});
	}
